document.addEventListener("DOMContentLoaded", function() {
    ...
    if (headerPlaceholder) {
        const isInstitucional = headerPlaceholder.getAttribute('data-header-type') === 'institucional';
        const headerUrl = isInstitucional ? '/components/header-institucional.html' : '/components/header.html';
    ...
    btnFlutuante.href = "https://wa.me/5571992126570?text=Olá,%20Dra.%20Joanny!%20Vim%20pelo%20site%20e%20gostaria%20de%20atendimento.";
...
document.addEventListener('click', function(e) {
    ...
    if (target && target.href && target.href.includes('wa.me')) {
        // Dispara o evento de conversão do Google Ads apenas no clique
        if (typeof gtag === 'function') {
            gtag('event', 'conversion', {
                'send_to': 'AW-18216982370/ouswCIrb1bkcEOKuxO5D',
                'value': 1.0,
                'currency': 'BRL'
            });
            console.log('Conversão de WhatsApp disparada no Google Ads!');
        }
    }
});